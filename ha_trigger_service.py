"""
    ...
    Install Deps:
    python3 -m pip install homeassistant_api
...
def main():
    ...
    trigger_service(service=args.service, entity_id=args.entity, config=config)


def trigger_service(service, entity_id, config):
    """ Use the ha client lib to trigger the scene """
    client = HAClient(
        config['url'],
        config['token']
    )
    ...
    service_domain = services[service_name]
    getattr(service_domain, service_action).trigger(entity_id=entity_id)
...
def get_args():
    ...
    parser.add_argument('--config', metavar='config_file_path', help='Path to config file', required=False, default='/etc/ha_config')
    ...
    if len(args.service.split('.')) < 2:
        sys.exit('--service should be in the format of service_name.service_action i.e. media_player.turn_off')
